...
import { ArrowLeftRight, Circle, ClipboardCheck, CreditCard, Dumbbell, ExternalLink, Film, LayoutDashboard, LogOut, Menu, Newspaper, PersonStanding, Users, X } from "lucide-react";
...
const sections = [
  ...
  { label: "1:1 COACHING", items: [
    { href: "/coaching/overview", label: "ခြုံငုံကြည့်မယ်", hint: "ဒီနေ့လုပ်စရာ", icon: PersonStanding },
    { href: "/coaching/clients", label: "Clients", hint: "Payment နဲ့ progress", icon: Users },
    { href: "/coaching/workouts", label: "Plans", hint: "Workout၊ Meal၊ Template", icon: Dumbbell },
    { href: "/coaching/feedback-forms", label: "Check-in Form", hint: "မေးခွန်းတွေ ပြင်မယ်", icon: ClipboardCheck },
  ] },
...
export function DashboardShell({ children, sessionStarted }: { children: React.ReactNode; sessionStarted: string }) {
  ...
  const groupedPath = pathname.startsWith("/coaching/payments")
    ? "/coaching/clients"
    : pathname.startsWith("/coaching/meals") || pathname.startsWith("/coaching/templates") || pathname.startsWith("/coaching/exercises")
      ? "/coaching/workouts"
      : pathname;
  ...
  return <div className={styles.shell}>
    ...
    <aside className={styles.sidebar} data-open={open}>
      ...
      <nav className={styles.nav}>{visibleSections.map((section) => <div key={section.label} className={styles.navSection}><p>{section.label}</p>{section.items.map(({ href, label, hint, icon: Icon }) => { const selected = groupedPath === href || (href !== "/dashboard" && groupedPath.startsWith(href)); return <Link href={href} key={href} data-active={selected} onClick={() => setOpen(false)}><span className={styles.navIcon}><Icon size={18} /></span><span><strong>{href === "/dashboard" && workspace ? "Website ပြောင်းမယ်" : label}</strong><small>{href === "/dashboard" && workspace ? "တခြား Dashboard ကိုသွားမယ်" : hint}</small></span></Link>; })}</div>)}</nav>
