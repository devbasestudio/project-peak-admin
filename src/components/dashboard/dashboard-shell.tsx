...
import { Circle, CreditCard, Dumbbell, ExternalLink, LayoutDashboard, LogOut, Menu, Newspaper, Users, X } from "lucide-react";
...
export function DashboardShell({ children, sessionStarted }: { children: React.ReactNode; sessionStarted: string }) {
  ...
  return <div className={styles.shell}>
    ...
    <div className={styles.workspace}>
      <header className={styles.topbar}><div className={styles.topbarLeft}><button className={styles.menu} onClick={() => setOpen(true)} aria-label="Menu ဖွင့်မယ်"><Menu size={21} /></button><div><p>{active.hint}</p><h1>{active.label}</h1></div></div><div className={styles.topbarRight}><span className={styles.device}><Circle size={7} fill="currentColor" />Secure device</span><a href="https://project-peak-landing.vercel.app" target="_blank" rel="noreferrer">Main site <ExternalLink size={14} /></a><a href="https://project-peak-beta.vercel.app" target="_blank" rel="noreferrer">Workout <ExternalLink size={14} /></a></div></header>
